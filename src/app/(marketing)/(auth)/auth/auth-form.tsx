...
function getAuthErrorMessage(error: { status?: number; message?: string }): string {
  if (error.status === 400) {
    return "Invalid request. Please try again.";
  }
  
  if (error.status === 401) {
    return "Authentication failed. Please check your account.";
  }
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      >
        ...
        {isPendingGoogle ? "Signing in..." : "Continue with Google"}
      </Button>
      ...
      >
        ...
        {isPendingGitHub ? "Signing in..." : "Continue with GitHub"}
      </Button>
    </div>
  );
}